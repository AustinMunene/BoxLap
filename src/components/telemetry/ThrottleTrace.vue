<!--
  ThrottleTrace.vue
  X axis: distance (m), Y axis: throttle (0–100)
  Two area traces with opacity so both are visible.
-->
...
type TelemetryPoint = { distance_m: number; throttle: number | null }
    ...
    datasets: [
      {
        ...
        data: (props.dataA || []).map(p => ({ x: p.distance_m, y: p.throttle })),
        borderColor: props.driverA.color,
        backgroundColor: props.driverA.color + '66', // ~40% opacity
        borderWidth: 2,
        pointRadius: 0,
        tension: 0.25,
        fill: true,
      ...
      },
      {
        ...
        data: (props.dataB || []).map(p => ({ x: p.distance_m, y: p.throttle })),
        borderColor: props.driverB.color,
        backgroundColor: props.driverB.color + '66',
        borderWidth: 2,
        pointRadius: 0,
        tension: 0.25,
        fill: true,
...
const chartOptions = computed(() => ({
  ...
  scales: {
    ...
    y: {
      ...
      grid: { color: 'rgba(255,255,255,0.04)' },
      ticks: { color: '#666', font: { family: 'DM Mono', size: 10 } },
      title: { display: true, text: 'Throttle (%)', color: '#555', font: { size: 11 } },
    },
  },
}))
</script>
...
<template>
  <div class="chart-wrapper">
    ...
    <div v-else class="no-data">Select drivers and lap to view throttle trace</div>
  </div>
</template>
